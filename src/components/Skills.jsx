...
import { SiFlutter, SiDjango, SiPostgresql, SiJavascript, SiTailwindcss } from "react-icons/si";
...
function Skills() {
  const skills = [
    ...
    { icon: <FaPython className="text-5xl" />, name: "Python", color: "from-blue-600 to-blue-400" },
    ...
    { icon: <SiFlutter className="text-5xl" />, name: "Flutter", color: "from-blue-400 to-cyan-300" },
    { icon: <SiPostgresql className="text-5xl" />, name: "PostgreSQL", color: "from-blue-700 to-blue-500" },
    { icon: <SiTailwindcss className="text-5xl" />, name: "Tailwind", color: "from-cyan-400 to-cyan-600" },
    { icon: <FaGitAlt className="text-5xl" />, name: "Git", color: "from-orange-600 to-red-500" },
  ];
  ...
  return (
    <section id="skills" className="min-h-screen bg-gradient-to-b from-slate-900 to-slate-950 text-white py-20 overflow-hidden">
          ...
          {skills.map((skill, index) => (
            ...
            >
              ...
              <div className="relative bg-slate-900/60 backdrop-blur-sm border border-slate-700/50 group-hover:border-slate-600 p-8 rounded-2xl flex flex-col items-center gap-4 transition-all duration-300 h-full justify-center hover:bg-slate-900/80">
                <div className="text-slate-300 group-hover:text-white transition-colors duration-300 group-hover:scale-110 transition-transform">
                  {skill.icon}
                </div>
                <span className="text-sm font-semibold text-slate-300 group-hover:text-white transition-colors text-center">{skill.name}</span>
              </div>
            </motion.div>
          ))}
        </motion.div>
